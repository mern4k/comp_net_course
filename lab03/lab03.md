...
#### Решение
Один пакет состоит из $56 \cdot 8 = 448$ бит, пакет формируется за $\frac{448}{128000}$ секунд, задержка передачи равна $\frac{448}{10^6}$ секунд. Осталось сложить эти величины и задержку распространения, получится $3.5 + 0.448 + 5 = 8.948$ миллисекунд.
...
#### Решение
Задержка передачи равна $\frac{1}{100} \cdot 10^3 = 10$ миллисекунд, значит средняя общая задержка равна 20 миллисекундам. По формуле Литтла $a = \frac{N}{d} = \frac{10 + 1}{10 + 10} = 0.55$ пакетов в миллисекунду, то есть 550 пакетов в секунду.
...
#### Решение
1. В данной ситуации первый пакет будет отправлен с маршутизатора ранее, чем придет второй придет на маршутизатор, так как $R_s < R_c$, поэтому разница во времени прибытия будет только из-за времени ожидания второго пакета на сервере, а она равна $\dfrac{L}{R_s}$.

2. Да, второй пакет будет находиться в буфере по изложенным выше причинам (первый пакет не успеет полностью отправиться, когда придет второй). Теперь нужно посчитать значение $T$. Если бы второй пакет отправлялся сразу после первого, то его время ожидания было бы равно времени ожидания передачи первого пакета на маршутизаторе минус время ожидания второго пакета на сервере. Тогда чтобы избавиться от ожидания вообще, нужно просто отправить его через посчитанное время ожидания, то есть $T = \dfrac{L}{R_c} - \dfrac{L}{R_s}$.
...
#### Решение
1. Посчитаем время, за которое объект размером 0.85 Мбит дойдет от интернета до локальной сети: $\Delta = \frac{0.85}{15} = \frac{17}{300}$ секунд. 
2. Теперь общее среднее время ответа (у нас 16 запросов в секунду): $d_{ответа} = 3 + \frac{\frac{17}{300}}{1 - \frac{17}{300} \cdot 16} = 3 + \frac{17}{28} \approx 3.607$ секунд.
3. По сути теперь у нас два случая: для $0.4 \cdot 16 = 6.4$ объектов используем формулу из предыдущего задания, а для остальных время ответа от кэш-сервера. Подставим: $d_{интернет} = 3 + \frac{\frac{17}{300}}{1 - \frac{17}{300} \cdot 6.4} = 3 + \frac{170}{3000 - 1088} \approx 3.089$ секунд. Теперь $d_{кэш} =  \frac{0.85}{100} = 8.5$ миллисекунд. В данной ситуации общее среднее время ответа - взвешенная сумма с соотвествующими вероятностями, то есть $0.4 \cdot 3.089 + 0.6 \cdot 0.0085 = 1.2407$ секунд.